...
const DELIVERY_OPTIONS = [
    { id: 'standard', label: 'Standard', delay: '5‚dtì7 jours ouvr√©s', price: '‚Ç¨4.90' },
    { id: 'express', label: 'Express',  delay: '2‚dtì3 jours ouvr√©s', price: '‚Ç¨9.90' },
    { id: 'same_day', label: '24h',     delay: 'Livraison le lendemain', price: '‚Ç¨14.90' },
];

const STEPS = [
    { id: 1, label: 'R√©sum√©',   Icon: Package    },
...
const Order = () => {
    ...
    const itemPrice = 59;
    const deliveryPrice = parseFloat(selectedDelivery.price.replace('‚Ç¨', ''));
        ...
        <div className="order-root">
            ...
            <header className="order-header">
                <Link to="/editor" className="order-back-link">
                    <ArrowLeft size={18} /> Retour √† l'√©diteur
                </Link>
            ...
            <div className="order-body">
                ...
                <div className="order-content">
                    {/* ‚îdt‚îdt STEP 1: Summary ‚îdt‚îdt */}
                    {step === 1 && (
                        <div className="order-card">
                            <h2 className="order-card-title">R√©sum√© de la commande</h2>
                            <div className="order-summary-item">
                                ...
                                <div className="summary-details">
                                    ...
                                    <span className="summary-meta">Couleur #F6F8F8 ¬∑ D√©cal Logo Poitrine</span>
                                    <span className="summary-price">‚Ç¨{itemPrice}.00</span>
                                </div>
                                <div className="summary-qty">
                                    <span className="qty-label">Qt√©</span>
                                    ...
                                    {DELIVERY_OPTIONS.map(opt => (
                                        ...
                                        >
                                            <div className="delivery-radio">
                                                <span>{delivery === opt.id ? '‚óè' : '‚óã'}</span>
                                            </div>
                            ...
                            <div className="order-total-row">
                                <span>Sous-total</span><span>‚Ç¨{itemPrice}.00</span>
                            </div>
                            <div className="order-total-row">
                    ...
                    {/* ‚îdt‚îdt STEP 2: Delivery ‚îdt‚îdt */}
                    {step === 2 && (
                        <div className="order-card">
                    ...
                    {/* ‚îdt‚îdt STEP 3: Payment ‚îdt‚îdt */}
                    {step === 3 && (
                        <div className="order-card">
                            ...
                            <div className="order-secure-note">
                                üîí Paiement s√©curis√© SSL 256-bit ‚dtî Vos donn√©es sont prot√©g√©es.
                            </div>
                            <div className="order-total-row order-grand-total" style={{ marginTop: '1.25rem' }}>
                                <span>Total √† payer</span><span>‚Ç¨{total}</span>
                            </div>
                            <div className="order-nav-row">
                                <button className="order-back-btn" onClick={back}><ArrowLeft size={15} /> Retour</button>
                                <button className="order-next-btn" onClick={next}>
                    ...
                    {/* ‚îdt‚îdt STEP 4: Confirmation ‚îdt‚îdt */}
                    {step === 4 && (
                        <div className="order-card order-confirm-card">
                            ...
                            <h2 className="split-heading confirm-split-heading">
                                ...
                                <span className="sh-bold">confirm√©e !</span>
                            </h2>
                            <p className="confirm-sub">
                                Votre cr√©ation est en route vers la production.<br />
                                Un email de confirmation a √©t√© envoy√© √† <strong>{form.email || 'votre adresse'}</strong>.
                            </p>
                            <div className="confirm-order-num">
                                Num√©ro de commande : <strong>#CL-{orderNum}</strong>
                            </div>
                            <div className="confirm-delivery-info">
                                <Package size={16} />
                                Livraison {selectedDelivery.label} ‚dtî {selectedDelivery.delay}
                            </div>
                    ...
                    {step < 4 && (
                        <aside className="order-sidebar">
                            ...
                            <div className="sidebar-item">
                                <div className="sidebar-thumb" style={{ background: '#2B3580' }}>üëï</div>
                                <div>
                                    <p className="sidebar-item-name">Hoodie Classique</p>
                                    <p className="sidebar-item-meta">Taille M ¬∑ Regular</p>
                                </div>
                                <span className="sidebar-item-price">‚Ç¨59</span>
                            </div>
                            <div className="sidebar-divider" />
                            <div className="sidebar-line"><span>Livraison</span><span>{selectedDelivery.price}</span></div>
                            <div className="sidebar-line sidebar-total-line"><span>Total</span><span>‚Ç¨{total}</span></div>
                        </aside>
                    )}
                </div>
            </div>
